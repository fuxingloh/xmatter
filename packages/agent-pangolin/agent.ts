...
export class PangolinTokenList extends FileSystemAgent<Token> {
...
const agent = new PangolinTokenList();

const filePath = ".repo/pangolin.tokenlist.json";
console.log(`Processing pangolin.tokenlist.json...`);

let tokenList: TokenList;
try {
  tokenList = JSON.parse(await readFile(filePath, { encoding: "utf-8" })) as TokenList;
} catch (error) {
  console.error(`Failed to parse pangolin.tokenlist.json:`, error);
  process.exit(1);
}

if (!Array.isArray(tokenList.tokens)) {
  console.error(`No tokens array found`);
  process.exit(1);
}
